        ...
        implements PutFieldDataService, ApplicationContextAware
{
    ...
    private PutFieldDataHandler      tsPutFieldDataHandler;
    ...
    public PutFieldDataResult putFieldData(PutFieldDataRequest request,Map<Integer, Model> modelLookupMap, List<Header> headers)
    {
        ...
        for (PutFieldDataCriteria fieldDataCriteria : request.getPutFieldDataCriteria())
        {
            try
            {
                ...
                else if ( fieldDataCriteria.getFieldData().getField().getFieldIdentifier().getSource() != null
                        && fieldDataCriteria.getFieldData().getField().getFieldIdentifier().getSource()
                        .equals(FieldSourceEnum.PREDIX_TIMESERIES.name()) )
			    {
    ...
    private void processPredixTimeseries(PutFieldDataRequest request,
			Map<Integer, Model> modelLookupMap, List<Header> headers,
			PutFieldDataResult fullResult,
			PutFieldDataCriteria fieldDataCriteria) {
    	{
            PutFieldDataRequest singleRequest = makeSingleRequest(request, fieldDataCriteria);
            PutFieldDataResult singleResult = this.tsPutFieldDataHandler.putFieldData(singleRequest, modelLookupMap, headers, HttpMethod.POST);
