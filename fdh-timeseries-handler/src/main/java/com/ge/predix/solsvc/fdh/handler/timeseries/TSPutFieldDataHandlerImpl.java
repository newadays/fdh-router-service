...
public class TSPutFieldDataHandlerImpl implements PutFieldDataHandler {
	private static final Logger log = LoggerFactory
			.getLogger(TSPutFieldDataHandlerImpl.class);
	 ...
	 */
	@Override
	public PutFieldDataResult putFieldData(PutFieldDataRequest request,
		...
		try {
			...
			this.timeSeriesPutExecutor.putFieldDataProcessor(request,
					putFieldDataResult, headers);
			return putFieldDataResult;
		} catch (Throwable e) {
			String msg = "unable to process request errorMsg=" + e.getMessage()
					+ " request.correlationId=" + request.getCorrelationId()
					+ " solutionIdentifier" + request.getSolutionIdentifier()
					+ " request = " + request;
			log.error(msg, e);
