...
public class TimeseriesDataRetrievalStrategy implements FieldDataHandlerGetFieldData {
		...
		for (FieldDataCriteria criteria : request.getFieldDataCriteria()) {
			...
			if (asset != null) {
				...
				if (fieldUri.startsWith("/asset/assetTag/"))
					fieldUri = fieldUri.substring(14);
	...
	private Asset lookupAsset(FieldDataCriteria criteria, List<Header> headers) {
		...
		if (filter instanceof FieldFilter) {
			for (FieldIdentifierValue fieldIdentifierValue : ((FieldFilter) filter)
			...
			org.mimosa.osacbmv3_3.SelectionFilter osaFilter = ((OsaFilter) filter)
					.getSelectionFilter();
	...
	private List<Double> getData(String uri, String assetTagKey, String sourceTag, FieldDataCriteria criteria,
		...
		if (criteria.getFilter() != null && criteria.getFilter() instanceof OsaFilter
			...
			return getDataFromOsaFilter(uri, assetTagKey, sourceTag, criteria, map, headers);
				...
				&& criteria.getFilter() instanceof FieldFilter) {
			return getDataFromFieldFilter(uri, assetTagKey, sourceTag, criteria.getFilter(),
					headers);
		} else
			throw new UnsupportedOperationException(
					"filter type=" + criteria.getFilter() + " not supported");
	}
	 ...
	 * assetTagKey, String sourceTag, Filter filter,
	 * List<Header> headers) { try { TimeseriesQueryBuilder tsBuilder =
	 ...
	 * null; FieldFilter fieldFilter = (FieldFilter)
	 * filter; for (FieldIdentifierValue fieldIdentifierValue :
	 ...
	 * @param assetTagKey 
	 */
	private List<Double> getDataFromFieldFilter(String uri, String assetTagKey, String sourceTag,
			Filter filter, List<Header> headers) {
		try {
			Long startTime = null;
			Long endTime = null;
	...
	private List<Double> getDataFromOsaFilter(String uri, String assetTagKey, String sourceTag,
		...
		try {
			...
			FilterDefinition filterDefinition = (FilterDefinition) ((OsaFilter) criteria
					.getFilter()).getSelectionFilter();
			...
			// Filter
			if (filterDefinition != null && filterType != null
					&& filterType.equals(FilterType.ROW_FILTER)) {
				...
				return dpList;
			} else if (filterDefinition != null && filterType != null
					&& filterType.equals(FilterType.TIME_FILTER)) {
				...
				long anchorTimeInLocalTimeZone = filterDefinition.getEndDefinition().getTimeFilter()
				...
				if (mapStart != null) {
					...
					startTime = Long.valueOf(mapStart);
						...
						.getTimeBinary() != null) {
					anchorTimeInLocalTimeZone = filterDefinition.getStartDefinition().getTimeFilter()
							.getAnchorTime().getTimeBinary().longValue();
				...
				throw new UnsupportedOperationException(
						"filterType=" + filterType + " not supported");
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
