...
@Component(value = "dxAssetGetFieldDataHandler")
public class DxGetFieldDataHandlerImpl extends AbstractFdhRequestProcessor
        implements GetFieldDataHandler
{
    private static final Logger               log = LoggerFactory.getLogger(DxGetFieldDataHandlerImpl.class);

    @Autowired
    private FieldTypeFilterProcessor fieldTypeFilterProcessor;
    @Autowired
    private FieldFilterProcessor     fieldFilterProessor;
    @Autowired
            ...
            List<Header> headers)
    {
        ...
        {
            // FieldFilter looks a lot like a sql where clause
            this.fieldFilterProessor.validateFieldFilter(fieldDataCriteria);
            return this.fieldFilterProessor.retrieveModels(fieldDataCriteria, headers);
        }
        else if ( fieldDataCriteria.getFilter() instanceof FieldTypeFilter )
        {
            // FieldTypeFilter used for getting Field Definitions
            this.fieldTypeFilterProcessor
                    .validateFieldTypeFilter((FieldTypeFilter) fieldDataCriteria.getFilter());
            return this.fieldTypeFilterProcessor.retrieveModels(fieldDataCriteria, headers);
        }
